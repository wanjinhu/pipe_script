...
@Description : 对使用eggnog方式获取的kegg信息进行处理, 生成后续可以分析的KO基因表(KO_samples.csv)和pathway代谢通路表(pathway_samples.csv)
'''
...
class KeggCount(object):
    def __init__(self):
        ...
        self.kegg_db = "/root/pipe_script/metagenome/pipe_script/kegg/KEGG_KO_ENZYME_PATHWAY.txt"    # 原始的KO对应的KEGG信息表【功能：统计KO基因】
        self.kegg_pathway = "/root/pipe_script/metagenome/pipe_script/kegg/ko_pathway_uniq.txt"  # 处理过的ko代谢通路三个层级的信息表【功能：统计pathway代谢通路】
    ...
    def KO_count(self,df_gene,df_ko):
        ...
        @df_ko: 宏基因组分析得到的非冗余基因集对应的KO基因信息表
        """
    ...
    def path_conut(self,df_gene,df_pathway):
        ...
        @功能: 生成代谢通路pathway组成表
        @df_gene: 宏基因组分析得到的非冗余基因集在样本中的分布表
        @df_pathway: 宏基因组分析得到的非冗余基因集对应的ko的通路信息表
        """
